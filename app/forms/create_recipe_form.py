...
class RecipeForm(FlaskForm):
    title = TextField("title", validators=[DataRequired(), Length(min=1, max=100, message="Recipe title must be between 1 and 50 characters.")])
    ...
    description = TextAreaField("description", validators=[DataRequired(), Length(min=1, max=200, message="Recipe description must be between 1 and 200 characters.")])
